...
AIC(lin.mod) #1206.77
AIC(lin.mod) - AIC(piecewise.ocean) # 4.78
...
AIC(lm(y.lake ~ x.lake)) - AIC(piecewise2) #19.34
...
posthoc.ldoc.2
posthoc.ldoc.2.p <- posthoc.ldoc.2$box.name.2[,4]
p.adjust(posthoc.ldoc.2.p, "holm", 28)


#Test what is significantly different in terms of BDOC
box.name2 = as.factor(box.name2)
summary(aov.boxplot.bdoc <- aov(box.bdoc ~ box.name2)) #ANOVA, F = 16.47, df = 6, 184
...
summary(aov.boxplot.bdoc.2 <- aov(box.bdoc.2 ~ box.name.3)) #ANOVA, F(5, 141) = 9.16, p < 0.05 (not shown)
...
ldoc1 = mean(na.omit(box.ldoc[box.name == "4"])) #Lakes 5.17 ± 0.28
ldoc2 = mean(na.omit(box.ldoc[box.name == "5"])) #Lagoons 5.77 ± 2.38
ldoc3 = mean(na.omit(box.ldoc[box.name == "6"])) #Inland seas 6.66 ± 0.95
ldoc4 = mean(na.omit(box.ldoc[box.name == "7"])) #Estuaries 6.16 ± 0.53
ldoc5 = mean(na.omit(box.ldoc[box.name == "8"])) #Coasts 5.49 ± 0.71
ldoc6 = mean(na.omit(box.ldoc[box.name == "9"])) #Open ocean 5.19 ± 0.99
mean(na.omit(box.ldoc[box.name == "10"])) #Productive systems 16.3 ± 1.76%

#Mean values ± se for BDOC
bdoc1 = mean(na.omit(box.bdoc[box.name2 == "2"])) #Lakes 21.16 ± 1.88
bdoc2 = mean(na.omit(box.bdoc[box.name2 == "3"])) #Lagoons 19.46 ± 1.52
bdoc3 = mean(na.omit(box.bdoc[box.name2 == "4"])) #Inlands seas 16.79 ± 1.53
bdoc4 = mean(na.omit(box.bdoc[box.name2 == "5"])) #Estuaries 13.39 ± 1.84
bdoc5 = mean(na.omit(box.bdoc[box.name2 == "6"])) #Coasts 10.52 ± 1.23
bdoc6 = mean(na.omit(box.bdoc[box.name2 == "7"])) #Open ocean 3.87 ± 0.63 -> We use ldoc values since BDOC can't be lower than ldoc
...
conceptual <- data.frame(LDOC = c(ldoc1,ldoc2,ldoc3,ldoc4,ldoc5,ldoc6,NA),
                         BDOC = c(bdoc1,bdoc2,bdoc3,bdoc4,bdoc5,ldoc6,NA),
...
summary(aov.9d <- aov(DOC9 ~ DOC9d)) #ANOVA, F(5, 74) = 2.697, p< 0.05, Holm p = 0.37
...
#LDOC
length(na.omit(Prod.ldoc)) #Number under bloom conditions 28
...
length(na.omit(box.name[box.name == 8])) #Number of coasts 42
...
mean(na.omit(box.ldoc[box.name == 10])) #16.3%
sd(na.omit(box.ldoc[box.name == 10]))/sqrt(length(na.omit(box.ldoc[box.name == 10]))) #1.755%
...
#Weigthed slope for BC and NBC 62.48 (slope BC * nBC + slopeNBC * nNBC) / ntotal
#Find the centroids
...
#Plots
#png("./Figure 4 - Color.png",units = "in", width = 8, height = 10, res = 300) #Export in png
pdf("./Figure 4 - Color.pdf",width = 8, height = 10) #Export in pdf
#Outline parameters
par(mfrow = c(2,1))
par(mgp=c(2.3,1,0))
par(mar=c(3.3,4,1,2)) 
...
#png("./Figure 4 - BW.png",units = "in", width = 8, height = 10, res = 300) #Export in png
pdf("./Figure 4 - BW.pdf",width = 8, height = 10)
#Outline parameters
par(mfrow = c(2,1))
par(mgp=c(2.3,1,0))
par(mar=c(3.3,4,1,2)) 
...
BDOC.lake.all = c(100,100-LDOC.lake,100-BDOC.lake) 
...
G.unbin.lake <- nlsLM(BDOC.lake.all ~ G2*exp(-k2*laket) + G3,
                      data = unbin.lake,
                      start = list(G2 = 50, k2 = 0.02, G3 = 70),
                      lower = c(10,0.001, 50),
                      upper = c(100, .04, 80))
...
#Continuum model
Cont.unbin.lake <- nlsLM(BDOC.lake.all ~ 100*(a / (a + laket))^v,
                      data = unbin.lake,
                      start = list(a = 56, v = .1),
                      lower = c(-2,-2),
                      upper = c(60, 10))
...
AIC(Cont.unbin.lake) #173.5
AIC(G.unbin.lake) #169.7
BIC(Cont.unbin.lake) #177.3
BIC(G.unbin.lake) #174.8
...
BDOC.Coast.all = c(100,100-LDOC.Coast,100-BDOC.Coast) 
...
G.unbin.Coast <- nlsLM(BDOC.Coast.all ~ G2*exp(-k2*Coastt) + G3,
                       data = unbin.Coast,
                       start = list(G2 = 30, k2 = 0.02, G3 = 80),
                       lower = c(10,0.001,50),
                       upper = c(50, .04, 90))
...
Cont.unbin.Coast <- nlsLM(BDOC.Coast.all ~ 100*(a / (a + Coastt))^v,
                       data = unbin.Coast,
                       start = list(a = 56, v = .1),
                       lower = c(-2,-2),
                       upper = c(60, 10))
...
AIC(Cont.unbin.Coast) #271.5
AIC(G.unbin.Coast) #270.9
BIC(Cont.unbin.Coast) #276.6
BIC(G.unbin.Coast) #277.7

```

```{r}
#Create multi-G plot
png(file = "./Figure 5 - BW.png",unit = "in", res = 300, family = "ArialMT", width = 6.2, height = 3.33)
#pdf(file = "./Figure 5 - BW.pdf",family = "ArialMT", width = 6.2, height = 3.33)
par(mfrow = c(1,2))
par(mgp=c(2.5,1,0))
...
axis(2, at = c(60, 70, 80, 90, 100), labels = c("60", "70", "80", "90", "100"), las = 2)
