...
#Piecewise model
segmented.mod <- segmented(lin.mod, seg.Z = ~Ocean.time.break, psi=c(40))
...
piecewise2 <- lm(y.lake ~ x.lake *(x.lake < 1.556) + x.lake*(x.lake > 1.556))
#Get the regressions' coefficients
summary(lm(y.lake[x.lake < 1.556] ~ x.lake[x.lake < 1.556])) # p = 0.72
summary(lm(y.lake[x.lake > 1.556] ~ x.lake[x.lake > 1.556])) # p = 0.036
...
curve(5.55 + 0 * x, add= T, from=.95, to=1.556, lwd = 3)
curve(-1.47 + 11.212 * x, add= T, from=1.556,to=2.78, lwd = 3)
legend("topleft",
       legend = paste("Lakes"),
       col="black",
       pch=8,
...
summary(aov.boxplot.ldoc <- aov(box.ldoc ~ box.name)) #ANOVA, F8,501 = 10.86, p < 0.05
...
mean(na.omit(box.ldoc[box.name == "8"])) #Coasts 6.56 ± 0.92
mean(na.omit(box.ldoc[box.name == "9"])) #Open ocean 5.19 ± 0.99
mean(na.omit(box.ldoc[box.name == "10"])) #Productive systems 15.7 ± 1.89%
...
SLDOCCoast = c(mean(na.omit(box.bdoc[box.name2 == "6"]))-mean(na.omit(box.ldoc[box.name == "8"]))) #3.97%
SLDOCOO = c(mean(na.omit(box.bdoc[box.name2 == "7"]))-mean(na.omit(box.ldoc[box.name == "9"]))) #0%

#Create a matrix with the above values
conceptual <- data.frame(LDOC = c(5.17,5.77,6.66,6.16,6.56,5.19,NA),
...
text(x=1:9, y = -4,
     paste(c("n = 55", "65", "229", "3", "20", "53", "45", "15", "25")),
     cex=.7) 
...
BDOC.Estuaire <- semilabile.frac[semilabile.frac > 0 & Environment_2=="Inland_sea" & Environment != "Lake"]
log10.DOC.Estuaire <- log10(DOC_ini[semilabile.frac > 0 & Environment_2 == "Inland_sea" & Environment != "Lake"])
DOC.Estuaire <- DOC_ini[semilabile.frac > 0 & Environment_2=="Inland_sea" & Environment != "Lake"]
...
#BDOC ~ DOC in estuaries
lmEst <- lm(BDOC.Estuaire ~ log10.DOC.Estuaire)
summary(lmEst)
#BDOC ~ DOC in lakes
lm4 <- lm(BDOC.lake ~ log10.DOC.lake)
summary(lm4)

#Plot for LDOC with square rooted data
plot(ldoc.sqrt ~ time,
    xlim=c(10,3000),
    main="",
    xlab = "Initial DOC (µM)",
    ylab=expression(LDOC~"(%), "~sqrt("y")),
    cex=1.3,
    log="x",
    xaxt = "n",
    las = 1,
